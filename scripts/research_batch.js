...
const results = await parallel(chunks.map((chunk, i) => () =>
  agent(
    [
      ...
      ...chunk.map((c) => `  - ${c}`),
      ``,
      `*** CRASH-SAFETY — THE MOST IMPORTANT INSTRUCTION ***`,
      `This run can be killed at any moment by a usage limit. Work is ONLY safe once it is`,
      `on disk. Therefore, the INSTANT you finish researching and scoring each company —`,
      `before you start the next one — Write that single company's JSON object to:`,
      `    ${root}/industries/${industry}/batches/raw/<slug>.json`,
      `where <slug> is the company name lowercased, spaces and punctuation replaced by`,
      `hyphens (e.g. "Kambi Group" -> kambi-group.json, "evoke plc (888)" -> evoke-plc-888.json).`,
      `The file content is the company object alone — the same shape as one element of the`,
      `\`companies\` array, e.g. {"name": "...", "sector": "...", "w": [...], ...}.`,
      `Do NOT batch these writes. Do NOT wait until the end. One company researched = one`,
      `file written immediately. If you are killed after 3 of ${chunk.length}, those 3 must survive.`,
      `After all ${chunk.length} are written, ALSO return them in your final JSON as normal.`,
      ``,
      `METHOD per company:`,
